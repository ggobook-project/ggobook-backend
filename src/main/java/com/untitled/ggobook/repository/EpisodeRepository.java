...
public interface EpisodeRepository extends JpaRepository<Episode, Long> {
    ...
    // 🌟 2-1. [검수 기능 - 신규 추가] 특정 상태인 회차들을 페이징 처리하여 조회 (N+1 문제 방지를 위해 Content Fetch Join)
    @Query("SELECT e FROM Episode e JOIN FETCH e.content WHERE e.status = :status")
    Page<Episode> findByStatusWithContent(@Param("status") Status status, Pageable pageable);

    // 3. [스케줄러 핵심] 예약 시간이 '지금'을 지난 승인된 회차들 조회
    List<Episode> findByStatusAndScheduledAtBefore(Status status, LocalDateTime now);

    // 4. 상세 조회 시 지연 로딩(Lazy) 에러를 방지
